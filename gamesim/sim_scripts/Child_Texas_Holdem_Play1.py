...
def putParentData():
    ...
    """
    dataDir = root_dir + app_dir + sim_script_dir + data_dir
    fileName = 'parentData_CPU' + cpuNum + '.txt'
    
    with open(dataDir + fileName, 'w') as d:
        json.dump(parentData,d)
    """
    return parentData
...
def putParentData2(numberOfPlayers, cpuNum, parentData, save_game_data):
    ...
    try:
        ...
        if save_game_data:
            ...
            add_gameData = ("insert into gamesim_cpu1LoopData " 
...
"""    
CardRankNumbers, CardRanks, CardSuits, DistinctHoleHands, HandTypes \
     = getInitialData()
    
Players,Player_Wins,Player_Hands,HandType_Wins,HandType_Hands,\
    DistinctHoleHands2_Wins,DistinctHoleHands2_Hands,\
    DistinctHoleHands2_TiedWins = getInitialCounters()
"""
"""
def getInitialData():
    
    with open('initialData.txt','r') as d:
        initialData = json.load(d)


    CardRankNumbers = initialData['card_rank_numbers']
    CardRanks = initialData['card_ranks']
    CardSuits = initialData['card_suits']
    
    HandTypes = initialData['hand_types']

    DistinctHoleHands = initialData['permutations']

    return CardRankNumbers,CardRanks,CardSuits,DistinctHoleHands, \
        HandTypes
"""    
"""    
def getInitialCounters():
    with open('initialChildData.txt','r') as d:
        initialCounters = json.load(d)
        
        #runTime = initialCounters['run_time']
        
        Players = initialCounters['players']

        Player_Wins = initialCounters['player_wins']
        Player_Hands = initialCounters['player_hands']
        
        HandType_Wins = initialCounters['hand_type_wins']
        HandType_Hands = initialCounters['hand_type_hands']
        
        DistinctHoleHands2_Wins = initialCounters['hole_hand_wins']
        DistinctHoleHands2_Hands = initialCounters['hole_hand_hands']
        DistinctHoleHands2_TiedWins = \
        initialCounters['hole_hand_tied_wins']
        
        return Players,Player_Wins,Player_Hands,HandType_Wins,HandType_Hands,\
            DistinctHoleHands2_Wins,DistinctHoleHands2_Hands,\
            DistinctHoleHands2_TiedWins
"""            
